...
// Command ugallu-honeypot materialises declared decoy K8s resources
...
import (
	...
	"github.com/ninsun-labs/ugallu/operators/honeypot/pkg/honeypot"
)
...
func runMain() error {
	...
	flag.StringVar(&auditBusEndpoint, "audit-bus-endpoint", "ugallu-audit-detection-bus.ugallu-system.svc.cluster.local:8444",
	...
	log := ctrl.Log.WithName("ugallu-honeypot")
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElection:          true,
		LeaderElectionID:        "ugallu-honeypot-leader",
	...
	emitter, err := emitterv1alpha1.NewEmitter(&emitterv1alpha1.EmitterOpts{
		...
		AttestorMeta: sign.AttestorMeta{Name: "ugallu-honeypot", Version: version},
	})
	if err != nil {
		return fmt.Errorf("emitter setup: %w", err)
	}

	if err = honeypot.SetupWithManager(mgr, &honeypot.Options{
		...
		return fmt.Errorf("honeypot reconcilers setup: %w", err)
	}
