...
// TestMain bootstraps the envtest control plane shared by both the
// fake-client unit tests (which simply ignore envCfg) and the
// integration suite below. envtest is best-effort: when
// KUBEBUILDER_ASSETS is unset the integration tests skip rather than
// fail so the fake-client suite still passes locally.
func TestMain(m *testing.M) {
	...
	// (operators/audit-detection/pkg/auditdetection/engine).
